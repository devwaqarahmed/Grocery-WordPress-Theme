<?php
/**
 * The Core Functions for SKT Supermarket
 *
	...
	if ( have_posts() ) :
		...
		$testimonialoutput .= '
		   <div class="item">	
				...
				<div class="tm_thumb">'.get_the_post_thumbnail( get_the_ID(), array(80,80) ).'</div>
			</div>		  
			';
		endwhile;
...
function display_team_meta_box( $team ) {
	...
	$instagramlink = get_post_meta( $team->ID, 'instagramlink', true );	
	
	$pinterest = get_post_meta( $team->ID, 'pinterest', true );
	$pinterestlink = esc_url( get_post_meta( $team->ID, 'pinterestlink', true ) );
...
<table width="100%">
  ...
  <tr>
   ...
   <tr>
    ...
    <td width="40%"><input type="text" name="instagram" value="<?php echo $instagram; ?>" /></td>
    <td width="40%"><input style="width:500px;" type="text" name="instagramlink" value="<?php echo $instagramlink; ?>" /></td>
  </tr>
  <tr>
...
function add_team_fields_function( $team_id, $team ) {
    ...
    if ( $team->post_type == 'team' ) {
		...
		if ( isset($_POST['instagram']) ) {
            update_post_meta( $team_id, 'instagram', $_POST['instagram'] );
        }
		if ( isset($_POST['instagramlink']) ) {
            update_post_meta( $team_id, 'instagramlink', $_POST['instagramlink'] );
        }
	...
	if ( have_posts() ) {
		while ( have_posts() ) { 
			...
			if( $col == 1 ){
				...
				$ourtm .= '">';
			$ourtm .= '<div class="team-thumb"><a href="'.get_permalink().'" title="'.get_the_title().'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/team_thumb.jpg" />' );
	  			$ourtm .= '</a>'.'</div>';

			   $ourtm .= '<div class="team-infobox">';
			   $ourtm .= '<a href="'.get_permalink().'" title="'.get_the_title().'"><h5 class="team-title">'.get_the_title().'</h5></a>';
					if( $designation != '' ){ $ourtm .= '<span class="team-desig">'.$designation.'</span>';}
                $ourtm .= '</div>';
				
				$ourtm .= '<div class="social-icons team-socials">';
						if( $facebook != '' ){
		...
		<div class="cols4">
			...
			<div class="team-thumb">
				<img src="'.get_template_directory_uri().'/images/team-img1.jpg">
			</div></a>
			<div class="team-info-box">
				<h3 class="team-title"><a href="#">Alex Doe</a></h3>
				<div class="social-icons  team-socials">
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) {
		...
		while ( have_posts() ) { 
			...
			$lbposts .= '<div class="style1-post-thumb"><a href="'.get_permalink().'" title="'.get_permalink().'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/default-thumbnail.jpg">' ).'</a>';
			$lbposts .= '<div class="blog-date">
							<span class="date-large">'.get_the_time('d').'</span>
							<span>'.get_the_time('M').'</span></div>'.$marg_clr.'';
			$lbposts .= '</div>';				
			if ( has_post_thumbnail() ){
			$lbposts .= '</div>';
			}
			
				$lbposts .= '<div class="post_block_style1_info '.$marg_cls.'">';
				$lbposts .= '<h3><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h3>
				<p>'.wp_trim_words( get_the_content(), $excerptlength ).'</p>
				<div class="readmorepost"><a href="'.get_permalink().'">'.$readmoretext.'</a></div>
				</div></div>'.$marg_clr.'';
				$n++;
		}
	}else{
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) {
		...
		while ( have_posts() ) { 
			...
			}$lbposts .= '<a href="'.get_permalink().'" title="'.get_the_title().'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/default-thumbnail.jpg">' ).'</a>'; if ( has_post_thumbnail() ){ $lbposts .= '</div>'; } $lbposts .= '
			<div class="post_info">
				<div class="post_block_style2_meta">
                	<span><i class="fa fa-clock"></i>'.get_the_date('F j, Y').'</span>
            	</div>
				<h4><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h4>
				<a class="more" href="'.get_permalink().'">'.$complete['recentpost_block_button'].'</a> 
				</div></div>'.$marg_clr.'';
				$n++;
		}
	}else{
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) {
		...
		while ( have_posts() ) { 
			...
			if(has_post_thumbnail() ){
			$lbposts .= '<div class="style3thumb"><a href="'.get_permalink().'" title="'.get_the_title().'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/default-thumbnail.jpg">' ).'</a></div>'; 
			}
					if(has_post_thumbnail() ){
  ...
  <div class="timeline-row">
	...
	if ( have_posts() ) {
		...
		while ( have_posts() ) { 
			...
			<div class="timeleft"><a href="'.get_permalink().'" title="'.get_the_title().'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/default-thumbnail.jpg">' ).'</a></div>'; 
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) :
		while ( have_posts() ) : the_post();
			...
			$postoutput .= '<li><a href="'.get_the_permalink().'">'.$imgUrl.'</a><a href="'.get_the_permalink().'">'.get_the_title().'</a></li>';
		endwhile;
	endif;
	wp_reset_query();
	$postoutput .= '</ul></div>';
...
// Shortcode Promobox1
/* [promobox1 bgcolor="#f7f7f7" topbordercolor="#ffc63b" otherbordercolor="#e8e6e6"]Description[/promobox1] */
...
// Shortcode Promobox2
/* [promobox2 bgcolor="#f7f7f7" leftbordercolor="#ffc63b" button="Hello Text" url="#"]Description[/promobox2] */
...
// Shortcode Promobox3
/* [promobox3 bgcolor="#f7f7f7" bottombordercolor="#ffc63b" button="Hello Text" url="#"]Description[/promobox3] */
...
// Shortcode Promobox4
/* [promobox4 bgcolor="#f7f7f7" bordercolor="#ffc63b"]Description[/promobox4] */
...
// Working Hours
/*[working-hours day="Monday - Friday" time="10.00am - 8.00pm" daycolor="#ffffff" timecolor="#ffc63b" bordercolor="#292d3a"]*/
...
function timingcal($skt_var){
		...
		return '<div class="timingbox">
				<div class="openingday"><p>'.$day.'</p></div>
				<div class="openingtime"><p class="'.$highlight.'">'.$hours.'</p></div>
				<div class="clear"></div>
				</div>';
}
add_shortcode('timing','timingcal');
...
function search_form() {
    $url = get_site_url();
    $form = '<form role="search" method="get" class="searchbox" action="'.$url.'">		
...
/* New Product  */
//[newproduct filter="true" show="8" offersale="SALE!" buttonname="ADD TO CART"]
	...
	<div class="skt-home-posts-column">
    	<div class="skt-home-posts-inner">
        	<div class="skt-home-posts-thumb">
                ...
                <?php } else { ?>
                    <a class="skt-home-posts-overlay" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/default-thumbnail.jpg"></a>
                <?php } ?>
			</div>
